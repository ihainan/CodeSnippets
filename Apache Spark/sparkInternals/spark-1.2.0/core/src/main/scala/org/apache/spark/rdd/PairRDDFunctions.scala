  ...
  with Serializable
{
  ...
  def groupByKey(partitioner: Partitioner): RDD[(K, Iterable[V])] = {
    ...
    /* I: 创建集合函数。CompactBuffer 可被理解为优化过的 ArrayBuffer，CompactBuffer 函数创建一个只包含元素 v 的 CompactBuffer */
  ...
  def join[W](other: RDD[(K, W)], partitioner: Partitioner): RDD[(K, (V, W))] = {
    /* I: 先 cogroup 再 flatMapValues，内部是 flatMapValues */
    /* I: this.cogroup(other, partitioner).collect => Array( key, (CompactBuffer(v11, v12, ..), CompactBuffer(v21, v22, ...)) ) */
    /* I: flatMapValues 内：pair => CompactBuffer */
    this.cogroup(other, partitioner).flatMapValues( pair =>
      for (v <- pair._1; w <- pair._2) yield (v, w) /* ArrayList((v1, w1), (v1, w2), ..)，再拆解得到每一个 (v, w)，最后得到 k, (v, w) */
    )
  }
